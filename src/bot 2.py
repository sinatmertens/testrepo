...
logging.basicConfig(filename='instagram_analyse.log',level=logging.WARNING, format='%(asctime)s %(message)s', datefmt='%Y-%m-%d %I:%M:%S')
...
class Bot:
    ...
    def login(self, username, password):
        ...
        time.sleep(2)
        self.driver.find_element_by_xpath("//button[contains(.,'Accept')]").click()
        time.sleep(2)
        self.driver.find_element_by_xpath("//button[contains(.,'Log In')]").click()
        notNow = self.driver.find_element_by_xpath("//button[contains(.,'Not Now')]")
        notNow.click()

    # "Get my Followers" geht gerade auf das Profil der Variable "platzhirsch" und scrapt alle Profile,
    # denen dieses Profil folgt.

    def get_my_followers(self, username):
        platzhirsch = ' '
        self.go_to_page("https://www.instagram.com/" + platzhirsch + "/")
        time.sleep(5)
            ...
            while(next):
                ...
                if(n_li_1 != n_li_2):
                    ...
                    for follow in following:
                        el = follow.find_element_by_tag_name('a')
                        profile = el.get_attribute('href')
                        my_followers_set.add(profile)
                else:
                    next = False

            return list(my_followers_set)   # Funktioniert!

    def get_followers(self, my_followers_arr, start_profile, relations_file): # Alle Relations
        ...
        for current_profile in my_followers_arr[start_profile - 1 : -1] + [my_followers_arr[-1]]:
            ...
            with open('./jquery-3.3.1.min.js', 'r') as jquery_js:
                ...
                try:
                    ...
                    print(current_profile + 'seems to be private Profile.')
                    count_my_followers += 1
                    time.sleep(10)
                    logging.warning(current_profile + 'seems to be private.')
                ...
                # if a set contains x then next = False
                ...
                following = self.driver.find_elements_by_class_name("FPmhX.notranslate._0imsa")
                for follow in following:
                ...
                print("This person follows " + str(len(follow_set)) + " accounts. \n")
